...
public class TheHeiganDance {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (isInRange(spellRow, spellCol, playerRow, playerCol)) {
                if (!isInRange(spellRow, spellCol, playerRow - 1, playerCol)
                        && isInBounds(playerRow - 1)) {
                    playerRow--;
                    spell = "";
                } else if (!isInRange(spellRow, spellCol, playerRow, playerCol + 1)
                        && isInBounds(playerCol + 1)) {
                    playerCol++;
                    spell = "";
                } else if (!isInRange(spellRow, spellCol, playerRow + 1, playerCol)
                        && isInBounds(playerRow + 1)) {
                    playerRow++;
                    spell = "";
                } else if (!isInRange(spellRow, spellCol, playerRow, playerCol - 1)
    ...
    public static boolean isInRange(int spellRow, int spellCol, int playerRow, int playerCol) {
